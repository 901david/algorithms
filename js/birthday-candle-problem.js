...
const birthdayCakeCandles = arr =>
  Array.from(
    arr
      .reduce((freqCounter, candle) => {
        if (freqCounter.has(candle)) {
          let prev = freqCounter.get(candle);
          freqCounter.set(candle, prev + 1);
        } else {
          freqCounter.set(candle, 1);
        }

        return freqCounter;
      }, new Map())
      .values()
  ).reduce((max, num) => (num = Math.max(max, num)), -Infinity);
